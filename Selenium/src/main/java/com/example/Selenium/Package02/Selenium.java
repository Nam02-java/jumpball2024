...
public class Selenium extends TelegramLongPollingBot {
    ...
    public ResponseEntity<?> Direction(@RequestParam Map<String, String> params) throws InterruptedException, IOException, TelegramApiException {
...
//            String text = getChar(TextCSV, 1, 20);
//            arrayList.add(text);
//            System.out.println(text);
            ...
            ExecutorService executor = Executors.newFixedThreadPool(arrayList.size());
...
//                            params.put("Text", arrayList.get(j));
//                        }
...
//            }
            ...
            for (int i = 0; i < arrayList.size(); i++) {
                final int index = i; // Khai báo một biến cuối cùng để sử dụng trong phạm vi luồng
                executor.execute(() -> {
                    try {
                        params.put("Text", arrayList.get(index)); // Sử dụng chỉ số i để lấy giá trị từ arrayList
                        params.put("Voice", VoiceCSV);
                        params.put("FileName", FileNameCSV);
                        MultiDriver(params);
                    } catch (InterruptedException e) {
    ...
    private static String getChar(String text, int start, int end) {
        StringBuilder result = new StringBuilder();
        for (int i = start - 1; i < end && i < text.length(); i++) {
            result.append(result.charAt(i));
        }
        return result.toString();
    }

    @GetMapping("/MultiDriver")
    public ResponseEntity<?> MultiDriver(@RequestParam Map<String, String> params) throws InterruptedException, IOException, TelegramApiException {
